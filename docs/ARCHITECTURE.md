...
[UrlShortenerService]  ── business rules: alias validation, expiry, collision handling
  │           uses → [Base62Encoder] (Base62 util, stateless)
  ▼
...
| `config` | `OpenApiConfig`, `RedisConfig`, `RateLimitConfig` | — |
| `util` | `Base62Encoder`, stateless helpers | — |
| `safety` | Pluggable provider HTTP transport and DNS resolver contracts | `exception` |

Controllers depend on services and DTOs; services use repositories and infrastructure boundaries. This is a Spring-coupled layered monolith, not a framework-independent core. Domain entities depend on JPA annotations.

## 4. Data model
**Table: `short_url`**
| Column | Type | Notes |
|---|---|---|
| `id` | `BIGINT GENERATED BY DEFAULT AS IDENTITY PK` | internal id, also seeds Base62 code for the default (non-custom-alias) path |
...
- Default: direct Base62 encoding of the PostgreSQL identity using `0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz`. No padding or shuffle; generated codes are enumerable and are not an authorization control.
...
- **Config**: `application.yml` with the default host configuration and Spring profiles `docker`, `test`; secrets via env vars (`POSTGRES_URL`, `POSTGRES_USER`, `POSTGRES_PASSWORD`).
...
## 8. Deployment shape
`docker-compose.yml` starts app, PostgreSQL, and Redis. Docker stages are Node frontend build → Maven packaging → non-root JRE runtime. App waits for PostgreSQL and Redis health; Flyway migrates on boot. A trusted external scanner must be configured; the CI overlay supplies a test-only fixture.
...
with `noopener noreferrer`; analytics targets `/#/analytics?code=...`, which loads
the side-effect-free metadata endpoint for the selected API version. In V2 mode
the route includes `mode=v2` and opens in the same tab to retain the in-memory key. Clipboard
...
socket peer address using a shared secret of at least 32 bytes. The quota namespace never receives
...
The active Flyway location is `db/migration/postgres`: V1 identity/link schema,
V2 owners/keys/audit and owner-list index, V3 UTC timestamp/default correction,
V4 safety state and audit. Root/MySQL V1 files are inactive. The table above describes
core link fields; current links additionally hold owner_id, safety_state,
safety_provider, and scanned_at. API-key/safety repositories use JdbcTemplate.

Cached redirects compare the cached destination against the active, unexpired
PostgreSQL row before recording access. A mismatch evicts/falls back to a locked
lookup, including alias reuse after failed eviction. PostgreSQL still receives a
write for every redirect. Future editing requires explicit cache invalidation.

Redirect Redis values contain full destinations; quota keys use HMAC identifiers,
and safety keys use URL hashes. These namespaces have different privacy properties.
DestinationPolicy has a fixed 2048-character ceiling in addition to the service's
configurable maximum; raising the latter does not raise the effective ceiling.
The existing CORS source bean is not wired to a filter/MVC registration. Supported
frontend deployment is same-origin; Vite uses its proxy. Cross-origin support must
be separately implemented and tested, not inferred from that bean.

Creation/analytics/deletion select V1 or V2 through the shared management request
helper. AppShell holds the V2 key only in memory and remounts management views
when access changes; requests never fall back to V1 on authentication failure.
My links uses bounded V2 pagination (20 rows per page). QR uses V2 and reuses
the shared key when present, with its separate key field retained in legacy mode. QR preview uses metadata plus
PNG and SVG, consuming three management admissions. Metadata omits safety state;
